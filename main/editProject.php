...
if (isset($_POST['key'])) {
  ...
  $user->show('projects', 'id', $user->showprojectid());
  $project_data=$user->data();
  if(Input::exists()){
  if(Token::check(Input::get('token'))){
    ...
    $validation=$validate->check($_POST,array(
      ...
      if($validation->passed()){
        ...
        try{
          $user->update('projects',$user->showprojectid(),array(
            'name'=>Input::get('name'),
            'project_key'=>Input::get('project_key')
          ));
          
        echo '<script type="text/javascript"> alert("Updates successfully");';
  ...
  ?>
    ...
    <form action="" method="post">
       ...
       <label for="name"> Name</label>
        <input type="text" id="name" name="name" placeholder="Enter project name"  value="<?= escape( $project_data->name) ?>" required>
       </div>
        <div class="design">
        <label for="project_key">Key</label>
        <input type="text" id="project_key" name="project_key" size="5" maxlength="5" placeholder="e.g. 'TEST " value="<?= escape($project_data->project_key)?>">
        </div>
        <div class="design">
